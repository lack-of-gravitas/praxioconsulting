...
import { fetchGetJSON } from "@utils/helpers";
...
export async function getStaticPaths(context) {
  ...
  let dbQuery = `
  query getSlugs ( $brand: String!) {slugs: globals (where: {domain: $brand}) {pages { slug }}}`;
  ...
  fetch(`${process.env.NEXT_PUBLIC_BACKEND}/graphql`, req)
    ...
    .then((results) => {
      ...
      // exclude blog and other pages that are built with their own qualified paths

      let slugArray = [];

      // generate all pages paths
      results.data.slugs[0].pages.forEach((page) => {
        slugArray = []; // reset temp array
        slugArray.push(page.slug);
      ...
      results.data.slugs[0].products.forEach((product) => {
        slugArray = []; // reset temp array
        slugArray.push("products", product.slug);
        data = [...data, { params: { slug: slugArray } }];

        // if (product.categories.slug === "") {
        //   // console.log("product.slug is empty");
        // } else {
