...
export function getStrapiMedia(url) {
  ...
  return `${process.env.NEXT_PUBLIC_BACKEND || "http://localhost"}${url}`;
}

export function getStrapiURL(path) {
  return `${process.env.NEXT_PUBLIC_BACKEND || "http://localhost"}${path}`;
}
...
export function getData(slug, locale, apiID, kind, preview) {
  ...
  if (kind == 'collectionType') {
    ...
    if (apiID == 'page') {
      ...
      //TODO: check if course or service
      prefix= `/products`;
    }
  ...
  // else {
  //   // logic for singleton pages which we do not have so does not apply
